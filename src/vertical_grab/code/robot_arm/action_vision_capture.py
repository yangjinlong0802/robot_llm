...
_PARENT = os.path.dirname(_THIS_DIR)           # vertical_grab/code
_GRAND_PARENT = os.path.dirname(_PARENT)       # vertical_grab
_ROBOT_ARM_DIR = _THIS_DIR
# 与 grab_pingzi_baizhuo.py 一致：调试图默认写到 code/pictures
_PICTURE_DIR_DEFAULT = os.path.normpath(os.path.join(_THIS_DIR, "..", "pictures"))

def _ensure_path(p: str) -> None:
    if p not in sys.path:
        sys.path.insert(0, p)

_ensure_path(_ROBOT_ARM_DIR)
_ensure_path(_PARENT)
_ensure_path(_GRAND_PARENT)
...
def load_grab_pingzi_module():
    """
    从与本文件同目录加载 grab_pingzi_baizhuo.py（你在 d:\\rm\\...\\robot_arm 下的那份脚本）。
    若文件不存在则返回 None。
    """
    import importlib.util

    path = os.path.join(_THIS_DIR, "grab_pingzi_baizhuo.py")
    if not os.path.isfile(path):
        return None
    spec = importlib.util.spec_from_file_location("grab_pingzi_baizhuo", path)
...
def run_pingzi_capture(controller, robot, width: int = 640, height: int = 480) -> bool:
    """
    执行与 grab_pingzi_baizhuo.capture_and_move 相同的「瓶子/白桌」抓取流程。
    优先加载同目录 grab_pingzi_baizhuo.py，否则尝试包内 import。
    """
    mod = load_grab_pingzi_module()
    if mod is not None and hasattr(mod, "capture_and_move"):
        return bool(mod.capture_and_move(controller, robot, width, height))
    try:
        import grab_pingzi_baizhuo as mod2

        return bool(mod2.capture_and_move(controller, robot, width, height))
    except ImportError as e:
        raise ImportError(
            "未找到 grab_pingzi_baizhuo.py：请复制到与 action_vision_capture.py 同一目录，"
            "或加入 PYTHONPATH。"
        ) from e
